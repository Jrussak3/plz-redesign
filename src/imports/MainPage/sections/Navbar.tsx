...
function NavLink({ label, to, variant = "desktop" }: { label: string; to?: string; variant?: "desktop" | "drawer" }) {
  if (variant === "drawer") {
    const content = (
...
function MobileMenuPanel({ open, onClose, onNavigate }: { open: boolean; onClose: () => void; onNavigate: () => void }) {
  useEffect(() => {
    if (!open) return;

...
import { type PersonaKey, type PersonaData, PERSONAS, PERSONA_ORDER } from "../data/personas";
...
function PersonaAvatar({ persona }: { persona: PersonaData }) {
  return (
...
function PersonaButton({ persona, isActive, onSelect }: { persona: PersonaData; isActive: boolean; onSelect: () => void }) {
  return (
    <button
      type="button"
      onClick={onSelect}
      ...
      className={
        isActive
          ? "bg-[#22005d] content-stretch cursor-pointer flex items-center justify-center min-h-[44px] md:min-h-[72px] py-[8px] md:py-[16px] relative rounded-[100px] shadow-[0px_20px_25px_0px_rgba(131,153,214,0.1),0px_8px_10px_0px_rgba(0,0,0,0.1)] shrink-0 w-auto md:w-full"
...
function PersonaList({ active, onSelect }: { active: PersonaKey; onSelect: (key: PersonaKey) => void }) {
  return (
    <div
...
export function PersonaSwitcherSection() {
  const [active, setActive] = useState<PersonaKey>("adam");

...
import { PRICING_PLANS, type PlanFeature, type PricingPlan } from "../data/pricingPlans";
...
function FeatureStatusIndicator({ feature }: { feature: PlanFeature }) {
...
const CARD_BACKGROUND: Record<string, string> = {
...
function PlanCardBody({ plan }: { plan: PricingPlan }) {
  return (
...
function PlanCard({ plan, className = "" }: { plan: PricingPlan; className?: string }) {
  if (!plan.featured) {
...
function MobileCarouselCard({ plan, isActive, isPeek, width, onClick }: { plan: PricingPlan; isActive: boolean; isPeek: boolean; width: number; onClick?: () => void }) {
  return (
    <div
      onClick={onClick}
      ...
      <div className="flex items-center justify-center gap-[16px] pt-[24px]">
        ...
        <div className="flex items-center justify-center gap-[10px]">
          {PRICING_PLANS.map((_, i) => (
            ...
            >
              <span className={`rounded-full bg-[#6750a4] transition-all duration-300 ${i === activeIndex ? "size-[14px]" : "size-[10px] opacity-50"}`} />
            </button>
          ))}
        </div>
        <button

...
import { TRUSTED_CITIES, type TrustedCity } from "../data/trustedCities";
...
function CityDot({ city, isActive, onHover, onLeave }: { city: TrustedCity; isActive: boolean; onHover: () => void; onLeave: () => void }) {
  return (
    <button
      type="button"
...
function CityCard({ city }: { city: TrustedCity }) {
  ...
  return (
    ...
    >
      {city.orgs.map((org) => (
        <div
          key={org.name}
          className="flex w-[130px] flex-col items-center justify-center gap-[4px] rounded-[8px] p-[8px] shadow-[0px_0px_5px_0px_rgba(51,51,51,0.3)]"
        >
